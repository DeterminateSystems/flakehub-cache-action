...
jobs:
  check:
    ...
    steps:
      ...
      - uses: DeterminateSystems/flakehub-cache-action@v3 # or v3.18.1 to pin to a release
      - run: nix build .
```
...
📍 Pinning to `DeterminateSystems/flakehub-cache-action@v3.18.1` guarantees:
...
|-----------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|--------------------------------------------|
| `use-gha-cache`             | Whether to upload build results to the Github Actions cache.<br />Set to `no-preference` or `null` to have the GitHub Actions cache turn on if it is available and FlakeHub Cache is not available (default).<br />Set to `enabled` or `true` to explicitly request the GitHub Actions Cache.<br />Set to `disabled` or `false` to explicitly disable the GitHub Actions Cache.
 |          |                                            |
